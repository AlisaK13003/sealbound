	...
	func clock_wise_rotation():
		for room_: dungeon_room in sub_rooms.values():
			...
			for dir in range(room_.required_directions.size()):
				room_.required_directions[dir] = (room_.required_directions[dir] + 1) % 4
...
var max_allowed_4_way_junctions: int
var max_allowed_3_way_junctions: int
var t_way_count: int = 0
var four_way_count: int = 0
...
func build_rooms(room_storage):
	...
	max_allowed_3_way_junctions = rng.randi_range(5, 20)
	max_allowed_4_way_junctions = rng.randi_range(2, 4)
	...
	var main_walker_lifetime: int = rng.randi_range(10, 17)
	...
	while not valid_critical_path_found:
		...
		var end_location = await walker_algorithm(main_walker_lifetime, walker_current_location, room_storage, true)
		...
		for dir in [0, 1, 2, 3]:
			...
			if room_storage.has(adjacent_pos):
				...
				if door_to_stairs in adjacent_room.required_directions:
					room_storage[end_location] = dungeon_room.new(end_location, [door_to_stairs], 1, "Down")
	...
	while not has_all_directions_been_satisfied:
		...
		for pos in num_of_other_walkers:
			...
			while not valid_path_found:
				...
				print("printing simulated storage")
				...
				print("printing simulated storage")
				print("number of rooms in simulated storage ", simulated_storage.size())
		...
		while not valid_position_was_found:
			...
			if just_placed_junction or walker_current_location == start_location or not dirs.has(currently_heading_in_x_direction):
				...
				if new_direction == currently_heading_in_x_direction:
					...
					steps_without_turning = 0 # Reset turning counter since we turned
			...
			if steps_without_turning > 2 and dirs.size() > 1:
				var new_direction = dirs.pick_random()
				...
				if room_chance < 0.1:
					if room_storage[walker_current_location].group_id == -1:
						...
						# If they were not able to find a spot that the 2x2 fit, then try to place something else
							...
							var new_room = dungeon_room.new(next_step_location, [same_directions[currently_heading_in_x_direction]], 1, "Generic_Path")

							var available_directions: int = 1
							var should_place_junction: bool = false

							if room_chance < (0.2 + steps_without_placing_junction * 0.3):
								should_place_junction = true
								new_room.total_direction_count = 3
							elif room_chance < (0.3 + steps_without_placing_junction * 0.3):
								should_place_junction = true
							...
							if should_place_junction:
								...
								for check_dir in DIR_VECTORS.values():
									if room_storage.has(next_step_location + check_dir):
							...
							if should_place_junction:
								...
								for dir_ in dir:
									if not room_storage.has(next_step_location + DIR_VECTORS[dir_]):
										...
										if room_storage[next_step_location + DIR_VECTORS[dir_]].required_directions.has(same_directions[dir_]):
											available_directions += 1
								...
								if available_directions < new_room.total_direction_count:
									new_room.total_direction_count = 2
									should_place_junction = false
								steps_without_placing_junction = 0
								
							room_storage[next_step_location] = new_room.duplicate()
							valid_position_was_found = true
							walker_current_location = new_room.room_pos
					else:
						var new_room = dungeon_room.new(next_step_location, [same_directions[currently_heading_in_x_direction]], 1, "Generic_Path")
						...
						valid_position_was_found = true
						walker_current_location = new_room.room_pos
						#place_generic_spot(currently_heading_in_x_direction, next_step_location, room_storage, false, room_chance, steps_without_placing_junction, valid_position_was_found)
				else:
					var new_room = dungeon_room.new(next_step_location, [same_directions[currently_heading_in_x_direction]], 1, "Generic_Path")
					...
					if room_chance < (0.25 + steps_without_placing_junction * 0.3) and t_way_count < max_allowed_3_way_junctions:
						should_place_junction = true
						new_room.total_direction_count = 3
					elif room_chance < (0.3 + steps_without_placing_junction * 0.3) and four_way_count < max_allowed_4_way_junctions:
						should_place_junction = true
						new_room.total_direction_count = 4
					else:
						should_place_junction = false
